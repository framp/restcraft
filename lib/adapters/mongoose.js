...
function queryHelper(funcs, $, model, cb){
  ...
  for (var i in queryParsing)
    if (!$[queryParsing[i]]) $[queryParsing[i]] = {};
  $.items[$.name] = $.items[$.name] || 10;
...
var limitPopulateTask = function($, result, callback){
  ...
  result.skip($.skip[$.name] ? $.skip[$.name] : ($.page[$.name]-1)*$.items[$.name]);
